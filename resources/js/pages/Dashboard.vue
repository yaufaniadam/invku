    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-8 p-6 md:p-8 text-foreground">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="card border-0 bg-card rounded-[24px] p-6 lg:p-8 shadow-card flex flex-col justify-between transition-transform hover:-translate-y-1 hover:shadow-xl">
                    <div class="flex items-center gap-4 border-b border-border/40 pb-4 mb-4">
                        ...
                        <div>
                            ...
                            <div class="inline-flex items-center gap-1 mt-1 rounded-full text-[11px] font-bold text-emerald-600 dark:text-emerald-500">
                                <TrendingUp class="h-3 w-3" /> +12.5%
                            </div>
                        </div>
                    </div>
                    <div>
                        <p class="text-2xl lg:text-3xl font-black text-foreground">{{ formatCurrency(stats.totalRevenue) }}</p>
                    </div>
                </div>
                ...
                <div class="card border-0 bg-card rounded-[24px] p-6 lg:p-8 shadow-card flex flex-col justify-between transition-transform hover:-translate-y-1 hover:shadow-xl">
                    <div class="flex items-center gap-4 border-b border-border/40 pb-4 mb-4">
                        ...
                        <div>
                            ...
                            <div class="inline-flex items-center gap-1 mt-1 rounded-full text-[11px] font-bold text-amber-600 dark:text-amber-500">
                                <TrendingUp class="h-3 w-3" /> +8.2%
                            </div>
                        </div>
                    </div>
                    <div>
                        <p class="text-2xl lg:text-3xl font-black text-foreground">{{ formatCurrency(stats.outstandingAmount) }}</p>
                    </div>
                </div>
                ...
                <div class="card border-0 bg-card rounded-[24px] p-6 lg:p-8 shadow-card flex flex-col justify-between transition-transform hover:-translate-y-1 hover:shadow-xl">
                    <div class="flex items-center gap-4 border-b border-border/40 pb-4 mb-4">
                        ...
                        <div>
                            <p class="text-xs font-bold uppercase tracking-wider text-muted-foreground">Total Invoice</p>
                            <div class="inline-flex items-center gap-1 mt-1 rounded-full text-[11px] font-bold text-blue-600 dark:text-blue-500">
                ...
                <div class="card border-0 bg-card rounded-[24px] p-6 lg:p-8 shadow-card flex flex-col justify-between transition-transform hover:-translate-y-1 hover:shadow-xl">
                    <div class="flex items-center gap-4 border-b border-border/40 pb-4 mb-4">
                        <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-purple-500/10 text-purple-500">
                            <Users class="h-6 w-6" />
                        </div>
                        <div>
                            <p class="text-xs font-bold uppercase tracking-wider text-muted-foreground">Klien Aktif</p>
                            <div class="inline-flex items-center gap-1 mt-1 rounded-full text-[11px] font-bold text-purple-600 dark:text-purple-500">
                                <TrendingUp class="h-3 w-3" /> +5.1%
                            </div>
                        </div>
                    </div>
                    <div>
                        <p class="text-2xl lg:text-3xl font-black text-foreground">{{ stats.activeClients }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 bg-card shadow-card rounded-[24px] overflow-hidden mt-2">
                <div class="card-header flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-border/40 bg-muted/10 px-8 py-6">
                    <div>
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="border-b bg-muted/20 text-xs font-bold uppercase tracking-wider text-muted-foreground/70">
                                ...
                                <th class="px-6 py-5">Klien</th>
                                <th class="px-6 py-5">Tanggal</th>
                                <th class="px-6 py-5 text-center">Status</th>
                                <th class="px-8 py-5 text-right border-l border-border/10">Jumlah</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-border/40">
                            <tr v-for="invoice in recentInvoices" :key="invoice.id" class="group transition-colors hover:bg-slate-50/50 dark:hover:bg-muted/10">
                                <td class="px-8 py-6">
                                    <Link :href="`/invoices/${invoice.id}`" class="font-bold text-primary hover:underline">
                                        {{ invoice.number }}
                                    </Link>
                                </td>
                                <td class="px-6 py-6 font-medium text-foreground">{{ invoice.client }}</td>
                                <td class="px-6 py-6 text-muted-foreground text-[14px] font-medium">{{ invoice.date }}</td>
                                ...
                                <td class="px-8 py-6 text-right font-black text-foreground text-lg border-l border-border/10 bg-muted/5">{{ formatCurrency(invoice.amount) }}</td>
                            </tr>

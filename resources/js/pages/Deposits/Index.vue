    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 sm:gap-8 p-4 sm:p-6 md:p-8 text-foreground">
            ...
            <div class="flex items-center justify-between gap-3">
                <div class="min-w-0">
                    <h1 class="text-2xl sm:text-3xl font-bold tracking-tight">Deposit Klien</h1>
                    <p class="text-muted-foreground mt-1 text-sm sm:text-base">Kelola saldo titipan dana klien.</p>
                </div>
                <Button @click="showForm = !showForm" class="h-10 sm:h-12 px-3 sm:px-6 rounded-xl font-bold shadow-lg shadow-primary/20 transition-all hover:scale-105 active:scale-95 shrink-0 text-sm" :variant="showForm ? 'outline' : 'default'">
                    <Plus v-if="!showForm" class="mr-1 sm:mr-2 h-4 w-4 sm:h-5 sm:w-5" />
                    <X v-else class="mr-1 sm:mr-2 h-4 w-4 sm:h-5 sm:w-5" />
                    <span class="hidden sm:inline">{{ showForm ? 'Batal Tambah' : 'Catat Deposit Baru' }}</span>
            ...
            <div v-if="clientBalances.length > 0" class="card flex flex-col items-center justify-between sm:flex-row gap-4 sm:gap-6 border-0 bg-primary/5 dark:bg-primary/10 rounded-[16px] sm:rounded-[24px] overflow-hidden p-4 sm:p-6 lg:p-8">
                <div class="flex items-center gap-4 sm:gap-6">
                    ...
                    <div>
                        <h2 class="text-lg sm:text-xl font-bold tracking-tight text-foreground">Ringkasan Saldo</h2>
                        <p class="text-xs sm:text-sm text-primary font-medium mt-1">
                            {{ clientBalances.length }} Klien memiliki dana titipan.
                        </p>
                    </div>
                </div>
            </div>

            <div v-if="clientBalances.length > 0" class="card border-0 bg-card shadow-card rounded-[16px] sm:rounded-[24px] overflow-hidden">
                    ...
                    <table class="w-full text-left text-sm sm:text-base">
                        <thead>
                            <tr class="border-b bg-muted/20 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-muted-foreground/70">
                                <th class="px-3 sm:px-8 py-3 sm:py-5">Nama Klien</th>
                                <th class="hidden sm:table-cell px-6 py-5">Total Disetor</th>
                                <th class="hidden md:table-cell px-6 py-5">Terpakai</th>
                                <th class="px-3 sm:px-8 py-3 sm:py-5 text-right border-l border-border/10 bg-primary/5 text-primary">Sisa Deposit</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-border/40">
                            <tr v-for="b in clientBalances" :key="b.client_id" class="group transition-colors hover:bg-slate-50/50 dark:hover:bg-muted/10">
                                <td class="px-3 sm:px-8 py-3 sm:py-6">
                                    <span class="font-bold text-foreground text-xs sm:text-sm">{{ b.client_name }}</span>
                                    <span class="sm:hidden block text-[10px] text-muted-foreground mt-0.5">Setor: {{ fmt(b.total_deposit) }}</span>
                                </td>
                                <td class="hidden sm:table-cell px-6 py-6 font-medium">{{ fmt(b.total_deposit) }}</td>
                                <td class="hidden md:table-cell px-6 py-6 font-medium text-muted-foreground">{{ fmt(b.used_deposit) }}</td>
                                <td class="px-3 sm:px-8 py-3 sm:py-6 font-black text-right text-sm sm:text-lg text-primary border-l border-border/10 bg-primary/5">
            ...
            <div class="flex items-center gap-2 sm:gap-4 mt-2 sm:mt-6">
                <div class="relative flex-1 min-w-0">
                    ...
                    <Input 
                        v-model="search" 
                        placeholder="Cari klien, nominal..." 
            ...
            <div class="card border-0 bg-card shadow-card rounded-[16px] sm:rounded-[24px] overflow-hidden">
                <div class="grid px-4 sm:px-8 py-4 sm:py-6 border-b border-border/40 bg-muted/10">
                    <h2 class="text-lg sm:text-xl font-bold">Riwayat Dana Masuk</h2>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm sm:text-base">
                        <thead>
                            ...
                            <tr v-for="d in deposits" :key="d.id" class="group transition-colors hover:bg-slate-50/50 dark:hover:bg-muted/10">
                                <td class="px-3 sm:px-8 py-3 sm:py-6">
                                    <div class="flex flex-col">
                                        <span class="font-bold text-foreground leading-tight text-xs sm:text-sm">{{ d.client_name }}</span>
                                        <span class="sm:hidden text-[10px] text-muted-foreground mt-0.5">{{ fmtDate(d.payment_date) }}</span>
                                    </div>
                                </td>
                                <td class="hidden sm:table-cell px-6 py-6 text-[14px] text-muted-foreground">{{ fmtDate(d.payment_date) }}</td>
                                <td class="hidden md:table-cell px-6 py-6 text-[14px] text-muted-foreground font-medium uppercase text-xs">{{ d.payment_method || '-' }}</td>
                                <td class="px-3 sm:px-8 py-3 sm:py-6 text-right font-black text-xs sm:text-base leading-none border-l border-border/10 bg-muted/5 text-primary">
                                    + {{ fmt(d.amount) }}
                                </td>
                            </tr>
                            <tr v-if="deposits.length === 0">
                                <td colspan="4" class="px-4 sm:px-8 py-16 sm:py-24 text-center">
                                    <div class="flex flex-col items-center gap-4 max-w-xs mx-auto">
                                        ...
                                        <div>
                                            <p class="text-lg sm:text-xl font-bold text-foreground">Riwayat tidak ditemukan</p>
                                            <p class="text-muted-foreground mt-1 text-sm">Belum ada riwayat dana masuk untuk klien manapun.</p>
                                        </div>
